  ...
  <div class="">
    ...
    <el-form size="small" :inline="true" style="margin-top: 10px;">
      ...
      <el-form-item class="el-item-right">
        <el-button type="primary" @click="showTaskConfig">分配明细</el-button>
        <el-button @click.stop type="text" size="small">
          <el-dropdown @command="(command) => { handleNewwork(command) }" trigger="click">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item v-for="(item, idx) in moreOption" :key="idx" :command="idx" v-if="idx != 0">
                <!-- {{ item }} :disabled="idx==1||idx==2&&checkIdArry.length==0" -->
                <el-button size="mini" type="text" v-if="idx==1||idx==2" :disabled="checkIdArry.length==0">{{ item }}</el-button>
                <el-button size="mini" type="text" v-else>{{ item }}</el-button>
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </el-button>
